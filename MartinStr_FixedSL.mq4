...
//|     Version：2026-08-13 1.00.002 (修复版)
//|     增加固定点数止损 ---- 27/07/2026
...
bool IsInBox() {
    ...
    // 改为 start=2：用 K[2] ~ K[BoxBars+1] 构成箱体，判断已收盘的 K[1] 收盘价是否在箱体内。
    if(Bars < BoxBars + 2)
        return true; // 历史K线不足，无法确定箱体；此处放行，后续方向判断会因K线不足返回0不开仓

    double highest = High[iHighest(NULL, 0, MODE_HIGH, BoxBars, 2)];
    double lowest  = Low[iLowest(NULL, 0, MODE_LOW, BoxBars, 2)];
